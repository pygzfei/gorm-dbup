...
// TrimSqlString 将sql里面注释和字符串，用空格替换
...
// Bytes2strUnsafe 高效的string和byte切片互转函数，但是依赖于go底层实现，不安全
...
//getPreNotZeroCount 获取字节前面二进制1个数
...
//seekToNext 获取字符串或者注释的右边界位置（不包含） rangeEnd是数组边界
...
//TrimSqlByteArray 将字节数组里面注释和字符串，用空格替换 rangeBeg和rangeEnd是数组元素起始位置 左闭右开
...
//ParseSql 拆分字节数组里面有效内容，注释和字符串 注意如果rangeBeg == rangeEnd 返回的是空数组
